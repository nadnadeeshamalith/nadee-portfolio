...
const translations = {
  si: {
    ...
    noAcc: "ගිණුමක් නැද්ද? ", hasAcc: " dදැනටමත් ගිණුමක් තිබේද? ",
...
const App = () => {
  ...
  const generateConcept = async () => {
    ...
    try {
      ...
      setAiConcept(data.candidates?.[0]?.content?.parts?.[0]?.text);
    } catch (error) {
      console.error(error);
    } finally {
      ...
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-6 text-center" style={{ fontFamily: "'Noto Sans Sinhala', sans-serif" }}>
        ...
        <div className="bg-slate-900/80 border border-slate-800 rounded-3xl p-10 max-w-lg w-full shadow-2xl">
          ...
          <div className="grid gap-4">
            {['si', 'en'].map(lang => (
              <button key={lang} onClick={() => setSelectedLanguage(lang)} className="p-4 rounded-xl bg-slate-950 border border-slate-800 hover:border-purple-500 text-xl font-bold text-slate-200 transition-all">
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-50 selection:bg-purple-500" style={{ fontFamily: "'Noto Sans Sinhala', sans-serif" }}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Noto+Sans+Sinhala:wght@300;400;500;600;700&display=swap');`}</style>
      ...
      <nav className={`fixed w-full z-50 transition-all ${isScrolled ? 'bg-slate-950/90 backdrop-blur-md py-3 shadow-xl' : 'bg-transparent py-5'}`}>
        <div className="container mx-auto px-6 flex justify-between items-center">
          ...
          <div className="hidden lg:flex gap-8 items-center">
            {['home', 'about', 'services', 'portfolio', 'contact'].map(sec => (
              <button key={sec} onClick={() => scrollToSection(sec)} className="text-sm font-medium hover:text-purple-400 transition-colors capitalize">{t('nav' + sec.charAt(0).toUpperCase() + sec.slice(1))}</button>
            ))}
            <div className="w-px h-6 bg-slate-800 mx-2"></div>
            {isLoggedIn ? (
              <button onClick={() => setShowLogoutConfirm(true)} className="flex items-center gap-2 text-red-400 hover:text-red-300 transition-colors text-sm font-bold">
                <LogOut size={18}/> {t('navLogout')}
              </button>
            ) : (
              <button onClick={() => setShowLogin(true)} className="flex items-center gap-2 text-slate-300 hover:text-white transition-colors text-sm font-bold">
                <User size={18}/> {t('navLogin')}
              </button>
            )}
            <button onClick={() => setSelectedLanguage(null)} className="p-2 bg-slate-800/50 hover:bg-slate-800 rounded-full transition-all"><Globe2 size={18} /></button>
          </div>
        ...
        {mobileMenuOpen && (
          <div className="lg:hidden bg-slate-900 border-t border-slate-800 p-6 space-y-4 absolute top-full left-0 w-full shadow-2xl animate-in slide-in-from-top duration-300">
            {['home', 'about', 'services', 'portfolio', 'contact'].map(sec => (
              <button key={sec} onClick={() => scrollToSection(sec)} className="block w-full text-left text-lg font-medium py-2">{t('nav' + sec.charAt(0).toUpperCase() + sec.slice(1))}</button>
            ))}
            <div className="h-px bg-slate-800 my-4"></div>
            {isLoggedIn ? (
              <button onClick={() => { setMobileMenuOpen(false); setShowLogoutConfirm(true); }} className="flex items-center gap-3 text-red-400 font-bold"><LogOut size={20}/> {t('navLogout')}</button>
            ) : (
              <button onClick={() => { setMobileMenuOpen(false); setShowLogin(true); }} className="flex items-center gap-3 text-white font-bold"><User size={20}/> {t('navLogin')}</button>
            )}
          </div>
        )}
      </nav>
        ...
        <div className="container mx-auto px-6 grid lg:grid-cols-2 gap-16 items-center relative z-10">
          <div className="space-y-8 animate-in fade-in slide-in-from-left duration-1000">
            ...
            <div className="flex flex-wrap gap-4 pt-4">
              <button onClick={() => scrollToSection('portfolio')} className="px-10 py-5 bg-white text-slate-950 rounded-2xl font-bold hover:scale-105 transition-all shadow-xl shadow-white/5">{t('btnViewWork')}</button>
              <button onClick={() => scrollToSection('contact')} className="px-10 py-5 border border-slate-700 rounded-2xl font-bold hover:bg-slate-900 transition-all">{t('btnContact')}</button>
            </div>
          </div>
          ...
          <div className="max-w-3xl mx-auto bg-slate-900 p-8 rounded-[32px] border border-slate-800 shadow-2xl">
            <div className="flex flex-col md:flex-row gap-4">
              ...
              <button 
                ...
                className="px-8 py-5 bg-gradient-to-r from-purple-600 to-purple-800 rounded-2xl font-bold flex items-center justify-center gap-3 hover:scale-105 transition-all shadow-lg shadow-purple-900/20 disabled:opacity-50"
              >
      ...
      <section id="portfolio" className="py-24 bg-slate-900/30">
        <div className="container mx-auto px-6">
          <div className="flex flex-col md:flex-row justify-between items-end gap-6 mb-16">
            ...
            <a href="https://youtube.com/@cinema24-h7v?si=bKOJtE1x8B9aQCEp" target="_blank" className="flex items-center gap-2 text-pink-500 font-bold hover:underline">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {portfolioItems.map(item => (
              <a 
                ...
                className="group relative overflow-hidden rounded-[32px] bg-slate-950 border border-slate-800 block aspect-video"
              >
                <img src={item.image} className="w-full h-full object-cover opacity-60 group-hover:opacity-30 transition-all duration-700 group-hover:scale-110" />
          ...
          <div className="mt-16 text-center">
             <a href="https://drive.google.com/drive/folders/1nLSW_lUYv-fv5Dcq-8ygfNGlCXsMql38?usp=sharing" target="_blank" className="px-12 py-5 bg-slate-800 hover:bg-slate-700 rounded-2xl font-bold transition-all inline-flex items-center gap-3">
            ...
            <div className="grid lg:grid-cols-2">
              <div className="p-12 lg:p-16 space-y-12">
                ...
                <div className="flex gap-4 pt-4">
                  {[Facebook, Instagram, Youtube, Linkedin].map((Icon, i) => (
                    <button key={i} className="w-12 h-12 bg-slate-800 rounded-xl flex items-center justify-center hover:bg-purple-600 hover:-translate-y-1 transition-all"><Icon size={20}/></button>
                  ))}
                </div>
              </div>
              ...
              <div className="bg-slate-800/50 p-12 lg:p-16 border-l border-slate-800">
                <form className="space-y-6" onSubmit={(e) => e.preventDefault()}>
                  ...
                  <button className="w-full py-5 bg-purple-600 rounded-xl font-bold flex items-center justify-center gap-3 hover:bg-purple-500 transition-all shadow-xl shadow-purple-900/20">
      ...
      {showLogin && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-950/80 backdrop-blur-md animate-in fade-in duration-300">
           <div className="bg-slate-900 p-10 rounded-[40px] max-w-md w-full border border-slate-800 relative shadow-2xl">
              <button onClick={() => setShowLogin(false)} className="absolute top-6 right-6 p-2 bg-slate-800 rounded-full hover:bg-slate-700 transition-all"><X size={20}/></button>
              ...
              <form onSubmit={handleLogin} className="space-y-4">
                ...
                <button type="submit" className="w-full py-5 bg-purple-600 hover:bg-purple-500 rounded-2xl font-bold mt-4 shadow-xl shadow-purple-900/20 transition-all">
                  {isLoginMode ? t('navLogin') : t('navSignUp')}
                </button>
              </form>
              
              <button onClick={() => setIsLoginMode(!isLoginMode)} className="w-full mt-6 text-sm font-medium text-slate-400 hover:text-purple-400 transition-colors">
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-950/80 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-slate-900 p-10 rounded-[32px] max-w-sm w-full border border-slate-800 text-center shadow-2xl">
            ...
            <div className="flex gap-4">
              <button onClick={() => setShowLogoutConfirm(false)} className="flex-1 p-4 border border-slate-700 rounded-2xl font-bold hover:bg-slate-800 transition-all">{t('btnNo')}</button>
              <button onClick={() => { setIsLoggedIn(false); setShowLogoutConfirm(false); }} className="flex-1 p-4 bg-red-600 hover:bg-red-500 rounded-2xl font-bold shadow-xl shadow-red-900/20 transition-all">{t('btnYes')}</button>
            </div>
          </div>
        </div>
      )}
